...
{#if isLoading}
  <div class="flex h-screen items-center justify-center bg-gray-50">
    <div class="flex flex-col items-center">
      <div class="h-12 w-12 animate-spin rounded-full border-4 border-blue-600 border-t-blue-100"></div>
        ...
        <div class="flex flex-1 flex-col overflow-y-auto">
          <nav class="flex-1 space-y-1 px-2 py-4">
            {#each navItems as item}
              ...
              >
                <div class="mr-3 flex-shrink-0 {isActive(item.href) ? 'text-blue-600' : 'text-gray-500 group-hover:text-gray-600'}">
                  {@html item.icon}
                </div>
                {item.name}
              </a>
